...
Simule une augmentation progressive du drift (épidémie de grippe).
"""
import pandas as pd
import numpy as np
...
def run_monitoring(reference, drift_factor):
    ...
    n_total = drift_metrics.get("number_of_columns", len(available))
    share = round(n_drifted / n_total, 2) if n_total > 0 else 0.0
